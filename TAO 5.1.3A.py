        ...
        try:
            ...
            if "*" in doc:
                if " " in doc:
                    ...
                    for i in range(tamanho):
                        ...
                        if aix5 > 0:
                            if aix5 == int(docm[i]):
                                ...
                                lp7 = aix5 % int(docm[i])
                                if aix5 >= int(docm[i]) and int(lp7) > 0 and lp7.is_integer() is True:
                                    aix5 = int(aix5)/int(docm[i])
                                # lp7=aix5%i
                                # aix5>=i and int(lp7)>0 and lp7.is_integer()==True:
                    continue
                elif len(doc) > 1:
                    doc.split()
                    docx = doc[1:]
                    ...
                    if aix1 >= int(int(doc[i])**2):
                        aix1 -= int(int(doc)**2)
                    if aix2 >= int(doc[i]):
                        aix2 -= int(doc)
                    if aix3 >= int(135-int(doc[i])):
                        aix3 -= int(135-int(doc[i]))
                    if aix4 >= int(int(doc[i])-180):
                        aix4 -= int(int(doc[i])-180)
                    if aix5 > 0:
                        if aix5 == int(doc[i]):
                            aix5 -= int(doc[i]):
                        else:
                            lp7 = aix5 % int(doc[i])
                            if aix5 >= int(doc[i]) and int(lp7) > 0 and lp7.is_integer() is True:
                                aix5 = int(aix5)/int(doc[i])
                    continue
                elif len(doc) == 1:
                    seta.clear()
                    dxcid.clear()
                ...
                if digprepre.isnumeric() == True:
                    ...
                    if "FIM" in digpre:
                        ...
                        if homm == 'A':
                            ...
                            '''
                            for i in range(aix6):
                                data = [int(seta[int(i)]), int(export1), int(export2), int(d1a), int(d1b), int(d1c), int(d2a), int(d2b), int(d2c), int(d3a), int(d3b), int(d3c), int(
                                    e1a), int(e1b), int(e1c), int(e2a), int(e2b), int(e2c), int(e3a), int(e3b), int(e3c), int(export3), int(export4), int(export5), int(aix1), int(aix2), int(aix3), int(aix4), int(aix5), int(aix6)]
                        ...
                        # REGISTRO DE PRESCRIÇÃO (DADOS INCOMPLETOS DE EXAME FÍSICO)
        ...
        try:
            ...
            if os.path.exists("registro_acupuntura.csv") == True and os.path.exists("ailog.csv") == True and os.path.exists("prontuario.csv") == True:
                ...
                if os.path.exists("ailog.csv") == False:
                    ...
                    time.sleep(1.5)
                    header = ['DIAGNÓSTICO', 'LÍNGUA', 'SEXO-IDADE', 'D1A', 'D1B', 'D1C', 'D2A', 'D2B', 'D2C', 'D3A', 'D3B', 'D3C', 'E1A', 'E1B', 'E1C', 'E2A', 'E2B', 'E2C', 'E3A', 'E3B', 'E3C', 'PULSOS', 'COMPLEIÇÃO', 'COMPLEIÇÃO RENYING',
...
atualizações = {
    ...
    'TAO 5.1.4': 'IMPLANTADO CÁLCULOS ARITMÉTICOS DE ANÁLISE AO BANCO DE DADOS',
}
