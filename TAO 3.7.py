    ...
    while True:
        ...
        if rede is True:
            ...
            if shu_h>18 and faselunar=='LUA NOVA': 
                warn_pun.add('⚠ ATENÇÃO! NÃO SEDAR')                                               
            if shu_h>18 and faselunar=='LUA CHEIA':
                warn_pun.add('⚠ ATENÇÃO! NÃO TONIFICAR')                 
            ...
            if rede is True:
                try:
                    ...
                    b2 = a1['temperature']
                    if int(b2)<20:
                        warn_pun.add('✘ TEMPERATURA MUITO BAIXA PARA ACUPUNTURA')
                    if int(b2)>31:
                        warn_pun.add('⚠ TEMPERATURA MUITO ALTA PARA MOXA')                    
                    b3 = a1['icon_num']
                    if int(b1)>90 and shu_h<18 and shu_h>16:
                        if b3!=27 and b3!=28:
                            warn_pun.add('⚠ EM BREVE: LUA OCULTA PARA ACUPUNTURA')                                          
                    if int(b1)>93 and shu_h>18:
                        if b3!=27 and b3!=28:
                            warn_pun.add('✘ LUA OCULTA PARA ACUPUNTURA')
                    if b3==35 or b3==36 or b3==32 or b3==31 or b3==30:
                        warn_pun.add('✘ LUA OCULTA PARA ACUPUNTURA')                 
                    if shu_h>16:
                        if shu_h<18:
                            if int(b3)!=26 or int(b3)!=27 or int(b3)!=28:
                                warn_pun.add('⚠ EM BREVE: LUA OCULTA PARA ACUPUNTURA')
                        if shu_h>18:
                            if int(b3)!=26 or int(b3)!=27 or int(b3)!=28:
                                warn_pun.add('⚠ LUA OCULTA PARA ACUPUNTURA')
        ...
        if rede is True:
            ...
            print('⚠ ATENÇÃO: SEM ACESSO A INTERNET')
        ...
        if homm == 'A' or homm=='D':
            ...
            if len(cpf)!=11 or cpf.isnumeric()==False:
                cls()
                print('\n\n\n\n\n\n\n')
                print('CPF inválido! \nVerifique número a mais ou a menos (retire traços e pontos)')
                time.sleep(2)
            ...
            if str(cpf)==str(tescpf):
                ...
                if os.path.exists('registro_acupuntura.csv') == True:
                    ...
                    if df.loc[df['CPF'].isin(["'"+cpf+"'"])].empty:
                        ...
                        print('CPF identificado em servidor!\n')
                        pacienteantigo=True
                else:
                    print('Base de dados inativa, não permite pesquisar CPFs!')
                    pacienteantigo=False
                if pacienteantigo==False:
                    print('Paciente novo! Iniciando cadastro do paciente!\n\n')
                time.sleep(1)
            else:
                cls()
                print('\n\n\n\n\n\n\n')
                                ...
                                if aco1!='':                                
                                    ...
                                    if int(idade) > 0:
                                        ...
                                        if len(aco4) > 7:
                                            ...
                                            print('ACOMPANHADA DURANTE A SESÃO DE: '+acompanhante)
                                            print('\n\nCADASTRO FINALIZADO')
                                            time.sleep(3)
                                            if homm == 'A':
                                                ben()
...
def ben():
    while True:
        ...
        if len(hda) > 311 or pacienteantigo==False:
            cls()
            print('\n\n\n\n\nESTE EXAME SERÁ AUTOMATICAMENTE REDIRECIONADO PARA QUESTIONÁRIO DE PROTOCOLOS DEVIDO A:\nPACIENTE NOVO, OU,\nDETECÇÃO DE PADRÃO NOVO\n\nO QUESTIONÁRIO BUSCARÁ PROTOCOLOS ESPECÍFICOS...\n')
            time.sleep(9)
            cls()
        if len(hda)==0:
            hda='HDA EM BRANCO'
        termoquery = input('\n\nOCORRE SINTOMAS TÉRMICOS? CALOR (SEM SER EM TODO CORPO), FRIO (SEM SER EM TODO CORPO), PELE SECA OU MUITO ÚMIDA OU MESMO EDEMA? (S/N) ').upper()
        global h3
        h3 = set()
        if termoquery == 'S':
        ...
        while True:
            try:                          
                ...
                if tipo_p[9] in pool or tipo_p[21] in pool or ne10=='S':
                    ne11=input('Irritação diária com diarréias que dura menos de 1 semana e passa sozinha porém recorrente? (S/N) ').upper()
                    if ne11=='S':
        ...
        while True:
            try:                          
                if tipo_p[13] in pool or tipo_p[12] in pool or ne10=='S':
                    ...
                    ne14=input('Tem tendência de irritar sem controle no momento e geralmente, em momentos de calma diários, existe uma vontade de vomitar que não é bem náusea? (S/N) ').upper()
                    if ne13=='S' or ne14=='S':
