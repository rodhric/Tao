...
def home():
    while True:
        ...
        valores.append(str('PRESCRITO(A) '+horadia+' HORA DO SHU ATUAL: ' +shu_agora+'ESTAÇÃO: '+estação))
        shu('')
        if os.path.exists('registro_acupuntura.csv') == True and os.path.exists('aculog.csv') == True:
            print('\n\nBase de dados: Ativa')
        else:
        ...
        print('[G] CONSULTAR DADOS')
        print('[H] IMPLEMENTAR BANCO DE DADOS')
                ...
                while True:
                    try:
                        ...
                        if sexo == 'M' or sexo == 'F':
                            ...
                            idd=int(idstr)
                            if idd>5 or idd<110:
                                valores.append('IDADE: '+str(idd)+' ANOS')
                                global acompanhante
                                if sexo == 'M':
                                    acompanhante='NÃO NECESSÁRIO ACOMPANHANTE'
                                    valores.append(acompanhante)
                                    print(acompanhante)
                                    print()
                                    time.sleep(3)
                                    break
                                if sexo == 'F':
                                    ...
                                    if aco2 !='':
                                        aco3=str(input('Data de nascimento (DD/MM/AAAA): '))
                                        if len(aco3)>=4:
                                            aco4=str(input('Telefone do acompanhante com DDD (xx)xxxx-xxxx (sem sinais): '))
                                            if len(aco4)>9:
                                                acompanhante=aco1+', '+aco2+', data de nascimento em '+str(aco3)+', telefone: '+str(aco4)
                                                valores.append('ACOMPANHADA DURANTE A SESÃO DE: '+acompanhante)
                            ...
                            if idd > 5 or idd < 110:
                                ...
                                if sexo == 'F':
                                    ...
                                    if aco2 != '':
                                        ...
                                        if len(aco3) >= 4:
                                            aco4 = str(
                                                input('Telefone do acompanhante com DDD (xx)xxxx-xxxx (sem sinais): '))
                                            if len(aco4) > 9:
        ...
        if homm=='G':
            cls()
            with open('base_dados_acupuntura.txt', 'r') as arquivo:
...
def ben():
    while True:
        ...
        valores.append('COMPLEIÇÃO: '+comfx)
        while True:
            try:
                comf = int(input(
        ...
        def f(x): return recun.add('EXAME DE RENYING: '+x)
        while True:
            try:
                print('\n\nEXAME DE RENYING: ')
                er = input('\nSE CUNKOU É MAIS FORTE:\nA-CUNKOU=2XRENYING\nB-CUNKOU=3XRENYING\nC-CUNKOU=4XRENYING\n\nSE REYING É MAIS FORTE:\nD-RENYING=2XCUNKOU\nE-RENYING=3XCUNKOU\nF-RENYING=4XCUNKOU\n\nZ-SEM ALTERAÇÕES DESCRITAS\n\n>>RESPOSTA: ').upper()
                print('\n\n')
                if len(er) == 1 and er.isnumeric()==False:
                    if er == 'Z':
                        f('EXAME DE RENYING NORMAL')
                        break
                    if er == 'A' or er == 'B' or er == 'C':
                        er1 = input(
                            'PULSO RENYING TEM PULSOS MAIS FORTES QUE OUTROS - COMO SE ? (S/N) ').upper()
                        if er1 == 'S' or er1 == 'N':
                            pass
                        else:
                            continue
                    ...
                    if er == 'D' or er == 'E' or er == 'F':
                        er2 = input(
                            'PULSO CUNKOU TEM PULSOS MAIS FORTES QUE OUTROS? (S/N) ').upper()
                        if er2 == 'S' or er2 == 'N':
                            pass
                        else:
                            continue
...
# -------------------------------------- LÍNGUA - PARTE 2
        ...
        if len(pureli)!=0:
            #map(lambda x: tensor.add(dx[x]), lista2a)
            valores.append('ANÁLISE DE LÍNGUA: '+str(pureli))
...
# -------------------------------------- VERIFICAÇÃO DE DIAGNÓSTICO - PARTE 4
        ...
        if len(excluesp) > 0:
            ...
            recun=recun.replace('{','')
            recun=recun.replace('}','')
            recun=recun.capitalize()                                 
            print(str(recun))
            for i, j in enumerate(list(excluesp), start=1):
                print(i, j)
            valores.append('SHAN HAN LUN: '+str(excluesp))
        ...
        print(tabela2)
        valores.append('PULSOLOGIA: METAL: '+d1a+'/'+d1c+' TERRA: '+d2a+'/'+d2c+' FOGO MINISTERIAL: '+d3a+'/'+d3c+' FOGO IMPERIAL: '+e1a+'/'+e1c+' MADEIRA: '+e2a+'/'+e2c+' ÁGUA: '+e3a+'/'+e3c)
            ...
            if len(seta) < 1:
                ...
                if digprepre.isnumeric() == True:
                    ...
                    if ' ' in digpre:
                        ...
                        continue
                    if 'FIM' in digpre:
                        valores.append('DIAGNÓSTICOS PRINCIPAIS: '+str(seta))
                        if len(acu2)!=0:
                            valores.append('ACUPONTOS PRESCRITOS: '+str(acu2))
                        print('\n\nSalvando dados...\n')
                        #with open('base_dados_acupuntura.txt','a') as arquivo:
                            #for valor in valores:
                                #arquivo.write(str(valor)+'\n')
                            #arquivo.write('____________________________________________________\n\n')
                        #for i in sorted(list(valores)):
                        for i, j in enumerate(list(valores)):
                            print(i, j)
                        print()
                        if len(pool3) < 1:
                            pool3.add('IGNORADO')
    ...
    while True:
        try:
            ...
            if os.path.exists('registro_acupuntura.csv') == True and os.path.exists('aculog.csv') == True:
                ...
                header=['NOME','DATA NASCIMENTO','PIP','CID-11','DOCUMENTO','DATA DE EMISSÃO','HASH']
                with open('pip_log.csv', 'w', encoding='UTF8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(header)
                # ARQUIVO PARA IMPLEMENTAÇÃO DE ALGORÍTMO DE ANÁLISE IA
                print('Criando arquivo de log verificador.csv...')
                header=['DATA','PRONTUÁRIO','CERTIFICADO','DOCUMENTO']
                with open('verificador.csv', 'w', encoding='UTF8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(header)         
