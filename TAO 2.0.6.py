...
def ben():
    while True:
        ...
        while True:
            try:
                fc=int(input('Digite a frequencia cardíaca (bpm) ou batimentos/incursão completa do examinador: '))
                if fc<10:
                    if fc<3:
                        rfc=1
...
# -------------------------------------- ENTRADA DE DADOS PARA PULSOS PATOLÓGICOS >>> DIR
        ...
        if rfc==1:
            print('Paciente apresenta tipo A em algum pulso, tente analisar corretamente!')
        if rfc==3:
            print('Paciente apresenta tipo B em algum pulso, tente analisar corretamente!')
        if antB == 'D':
            print(
                'Atenção nesta análise, paciente supostamente apresentará pulsos T ou U ou V. Tente fazer com mais lento o exame.')
        print()
...
# -------------------------------------- ENTRADA DE DADOS PARA PULSOS PATOLÓGICOS >>> ESQ
        ...
        print()
        print('Pulsos Fu, Chen, Shi, Kou, Huan, Lao, Ruo, Ru e Ji já são definidos por algorítmos. FC influencia em análise de pulsos Chi e Shu, podendo ficar indisponíveis.\n\n')
        if rfc==1 and 'A' not in ppd1 and 'A' not in ppd2 and 'A' not in ppd3:
            print('Paciente apresenta tipo A em algum pulso, tente analisar corretamente!')
        if rfc==3 and 'B' not in ppd1 and 'B' not in ppd2 and 'B' not in ppd3:
            print('Paciente apresenta tipo B em algum pulso, tente analisar corretamente!')
        if antB == 'D':
            print(
                'Atenção nesta análise, paciente supostamente apresentará pulsos T ou U ou V. Tente fazer com mais lento o exame.')
...
# -------------------------------------- VERIFICAÇÃO DE DIAGNÓSTICO - PARTE 4
        ...
        if 'C' in det_4:
            pool.add('DOR MUSCULAR POR E1 RÁPIDO')
        if 'C' in det_3:
            pool.add('RISO INCOERENTE, FOBIA SOCIAL E RISCO DE HÉRNIAS POR E1 LENTO')
        if 'C' in det_9:
            pool.add('SENSAÇÃO DE OBJETO EM GARGANTA POR E1 LONGO')
        if 'C' in det_15 or 'C' in det_14:
            pool.add('SÍNDROME DE DOR OBSTRUTIVA (BI) EM COSTAS POR E1 FORTE')
        if 'C' in det_12 or 'C' in det_10:
            pool.add('RISCO DE DIABETES, TENDÊNCIA A SOLUÇOS POR E1 PEQUENO (FINO)')
        if 'C' in det_7:
            pool.add('TENDÊNCIA A SEDE POR E1 ESCORREGADIO') 
        if 'C' in det_27 or 'C' in det_28 or 'C' in det_22 or 'C' in det_25 or 'C' in det_26:
            pool.add('TENDÊNCIA A MUDEZ, CEFALÉIA E HEMORRAGIAS POR E1 VARIÁVEL') 
        if 'P' in det_15 or 'P' in det_14:
            pool.add('TENDÊNCIA A EPILEPSIA E ESQUIZOFRENIA, CANSAÇO EM COSTAS E LESÕES REPETIDAS EM NARINAS POR D1 FORTE') 
        if 'P' in det_3:
            pool.add('DOR ESCAPULAR E SUDORESE DIFUSA POR D1 LENTO') 
        if 'P' in det_11:
            pool.add('DOR/INCHAÇO EM PERNAS/PÉ OU DOR EM COSTAS COM IRRITABILIDADE AO SOL POR D1 AMPLO') 
        if 'P' in det_12 or 'P' in det_10:
            pool.add('TENDÊNCIA A DIABETES OU DIARRÉIAS POR D1 PEQUENO')                                    
        if 'P' in det_7:
            pool.add('RESPIRAÇÃO RÁPIDA E REVERSÃO DE QI DE PULMÃO COM RISCOS DE HEMORRAGIAS POR D1 ESCORREGADIO') 
        if 'P' in det_27 or 'P' in det_28 or 'P' in det_22 or 'P' in det_25 or 'P' in det_26:
            pool.add('VÔMITOS COM SANGUE, ESCRÓFULA EM PESCOÇO OU AXILAS COM CÃIMBRAS POR D1 VARIÁVEL') 
        if 'F' in det_14:
            pool.add('AGRESSIVIDADE E VOZ ALTA POR E2 FORTE') 
        if 'F' in det_3:
            pool.add('VÔMITOS E DOR DISTENSIVA AO ALIMENTAR POR E2 LENTO') 
        if 'F' in det_11:
            pool.add('CARBÚNCULO, VÔMITOS, EPISTAXE E DOR EM FLANCO DIREITO OU DOR GENITAL COM LESÃO FUNDA OU DOR EM RIM AO TOSSIR POR E2 AMPLO') 
        if 'F' in det_9:
            pool.add('EXCESSO DE FOME E SEDE COM INCHAÇO MUSCULAR POR E2 DIMINUÍDO') 
        if 'F' in det_7:
            pool.add('INCHAÇO EM ESCROTO OU INCONTINÊNCIA URINÁRIA POR E2 ESCORREGADIO')
        if 'BP' in det_15 or 'BP' in det_14:
            pool.add('TREMOR DE EXTREMIDADES OU VÔMITOS APÓS CORRER E TENDÊNCIA A DIARRÉIA COM ESPUMA POR D2 FORTE')
        if 'BP' in det_3:
            pool.add('FRAQUEZA DE BRAÇOS E PERNAS COM TREMOR E FLACIDEZ POR D2 LENTO')
        if 'BP' in det_11:
            pool.add('TENDÊNCIA A DESMAIOS (SÍNDROME PIQI) E RISCO DE TUMORES DEVIDO A ESTASE POR D2 AMPLO')
        if 'BP' in det_10 or 'BP' in det_12:
            pool.add('INCHAÇOS MUSCULARES RECORRENTES POR D2 PEQUENO')
        if 'BP' in det_7:
            pool.add('INCHAÇO E QUEIMAÇÃO GENITAL COM DOR ABDOMINAL POR D2 ESCORREGADIO')
        if 'BP' in det_27 or 'BP' in det_28 or 'BP' in det_22 or 'BP' in det_25 or 'BP' in det_26:
            pool.add('ALTERAÇÃO GINECOLÓGICA E LESÕES OCULTAS ULCERADAS INTESTINAIS POR D2 VARIÁVEL') 
        if 'R' in det_14:
            pool.add('OSSOS FRACOS, PÉ FRIO E DURO, TENDÊNCIA A DELÍRIOS, RETENÇÃO FECAL E URINÁRIA POR E3 FORTE') 
        if 'R' in det_3:
            pool.add('DOR INTENSA EM COLUNA COM REFLUXO AOS ALIMENTOS POR E3 LENTO') 
        if 'R' in det_11:
            pool.add('TENDÊNCIA A IMPOTÊNCIA E HÉRNIAS ABDOMINAIS POR E3 AMPLO') 
        if 'R' in det_10 or 'R' in det_12:
            pool.add('DIARRÉIA COM URGÊNCIA E TENDÊNCIA A DIABETES POR E3 PEQUENO') 
        if 'R' in det_7:
            pool.add('DOR E EDEMA GENITAL, TENDÊNCIA A DESMAIO ORTOSTÁTICO (HIPOTENSÃO) POR E3 ESCORREGADIO')                                    
        if 'R' in det_27 or 'R' in det_28 or 'R' in det_22 or 'R' in det_25 or 'R' in det_26:
            pool.add('TENDÊNCIA A CARBÚNCULOS E HEMORRÓIDAS POR E3 VARIÁVEL') 
        if tipo_p[1] in pool:
            pool.add('FU - FLUTUANTE (PALPAÇÃO SUPERFICIAL QUE SOME AO APERTAR)= VENTO EXTERNO OU DEF YIN')
        if tipo_p[2] in pool:
            pool.add('CHEN - PROFUNDO (PALPAÇÃO QUASE EM OSSO), PROFUNDO-FRACO (XXI-RUO)= DEF QI E YANG, PROFUNDO-CHEIO (VI-SHI)= ESTASE QI E YANG OU FRIO/CALOR')
        if tipo_p[3] in pool:
            pool.add('CHI - LENTO (BRADICARDIA, <3BPM/IRPM DO EXAMINADOR), LENTO-FRACO (XXI-RUO)=FRIO VAZIO, LENTO CHEIO (VI=SHI)=FRIO CHEIO')
        if tipo_p[4] in pool:
            pool.add('SHU - RÁPIDO (TAQUICARDIA, >5BPM/IRPM EXAMINADOR), RÁPIDO-FLUTUANTE (I-FU)= CALOR VAZIO, RÁPIDO-CHEIO (VI-SHI)= CALOR CHEIO')
        if tipo_p[5] in pool:
            pool.add('XU - VAZIO (DIFICULDADE DE SENTIR, LARGURA AUMENTADA E MACIO) = DEF QI')
        if tipo_p[6] in pool:
            pool.add('SHI - CHEIO (NORMAL)')
        if tipo_p[7] in pool:
            pool.add(
                'HUA - DESLIZANTE (ESCORREGA NOS DEDOS, MÓVEL) = FLEUMA, UMIDADE OU GRAVIDEZ')
        if tipo_p[8] in pool:
            pool.add('SE - ÁSPERO (CRESPO/ÁSPERO, COM DENTES DE SERRA) = ESGOT. FLUIDO OU DEF. XUE, SE-15-XIAN OU SE-26-JIE = ESTASE XUE')
        if tipo_p[9] in pool:
            pool.add('CHANG - LONGO (AMPLA SENSIBILIDADE, BATEM ANTES DE APERTAR, TAMANHO VERTICAL/ALTURA COM MAIS PULSO) = CALOR')
        if tipo_p[10] in pool:
            pool.add('DUAN - CURTO (INVERSO DO IX-CHANG, OCUPA ESPAÇO MENOR QUE O HABITUAL) = DEF GRAVE DE QI (PIOR EM E)')
        if tipo_p[11] in pool:
            pool.add('HONG - TRANSBORDANTE (AUMENTO DE CALIBRE DO VASO, MAIS GROSSO, DERRAMADO, TAMANHO HORIZONTAL/LARGURA MAIOR) HONG-6-SHI= CALOR CHEIO OU FEBRE, HONG-5-XU= CALOR VAZIO, HONG-4-SHU=FLEUMA-FOGO, HONG-4-SHU-6-SHI OU HONG-25-CU-6-SHI= FOGO-CALOR CHEIO')
        if tipo_p[12] in pool:
            pool.add(
                'XI - FINO (MAIS FINO QUE O NORMAL) = ANEMIA/DEF XUE OU UMIDADE+DEF GRAVE DE QI')
        if tipo_p[13] in pool:
            pool.add('WEI - MÍNIMO (SEMELHANTE AO XII-XI, PORÉM MAIS FRÁGIL, FINO COMO UM CAPILAR) = DEF GRAVE DE QI E DE XUE')
        if tipo_p[14] in pool:
            pool.add('JIN - TENSO (TORCIDO COMO UMA CORDA GROSSA) JIN-1-FU-6-SHI=FRIO EXTERNO, JIN-6-SHI-2-CHEN=FRIO CHEIO, JIN-1-FU-2-CHEN= FRIO VAZIO OU DOR POR FRIO OU ASMA')
        if tipo_p[15] in pool:
            pool.add('XIAN - CORDA (MAIS FINO E MAIS TENSO QUE O XIV-JIN, FORÇA DA CORDA DE UM VIOLÃO) = DOR OU FLEUMA OU DEF XUE DE F, XIAN-6-SE OU XIAN-26-JIE = OBSTRUÇÃO DE QI COM OBSTRUÇÃO DE XUE DE CORAÇÃO')
        if tipo_p[16] in pool:
            pool.add('HUAN - RETARDADO = NORMAL, REPOUSO.')
        if tipo_p[17] in pool:
            pool.add('KOU - OCO (SUPERFICIALMENTE SENTIDO, AO APERTAR, INTERMEDIARIAMENTE PARA DE SENTIR E, VOLTA A SENTIR AO APERTAR PROFUNDAMENTE) = HEMORRAGIA, IMINÊNCIA DE RUPTURA DE HEMORRAGIA/CHOQUE. MÃO E PÉ GELADO: RISCO LETAL.')
        if tipo_p[18] in pool:
            pool.add('GE - EM COURO (DURO, TENSO E ESTICADO, ABERTO, COMO O TAMBOR DE COURO SUPERFICIALMENTE, PARECE VAZIO AO APERTAR (MAIOR VAZÃO) = DEF YUAN QI, DEF GRAVE DE YIN/R, BAIXA MING MEI (ANALISAR FACE)')
        if tipo_p[19] in pool:
            pool.add('LAO - FIRME (SENTIDO APENAS PROFUNDAMENTE, ROBUSTO, DURO, CHEIO MESMO APERTADO COM FORÇA, XIV-JIN-IX-CHANG) = ESTASE XUE, SE LAO-III-CHI= FRIO INTERNO, DOR')
        if tipo_p[20] in pool:
            pool.add('RU - ENCHARCADO (SEMELHANTE AO 1-F, PORÉM MACIO, MAIS ELÁSTICO QUE O HABITUAL E NÃO TOTALMENTE DEIXADO DE SENTIR AO APERTAR) = DEF QI E UMIDADE OU DEF YIN COM DEF YING QI QI')
        if tipo_p[21] in pool:
            pool.add('RUO - FRACO (NÃO SE SENTE SUPERFICIALMENTE, PORÉM SENTIDO INTERMEDIARIAMENTE E PROFUNDAMENTE) = DEF YANG OU DEF XUE')
        if tipo_p[22] in pool:
            pool.add('SAN - DISPERSO (VASO PEQUENO E SUPERFICIAL COM BATIMENTO EM PONTOS E NÃO INTEIRAMENTE, COMO SE TIVESSE QUEBRADO) = DEF QI E DEF XUE TAMBÉM COM DEF QI/R (CONDIÇÃO GRAVE)')
        if tipo_p[23] in pool:
            pool.add(
                'FUA - ESCORREGADIO (PROFUNDO E ADERIDO AO OSSO, SEM MOBILIDADE) DEF GRAVE DE YANG')
        if tipo_p[24] in pool:
            pool.add('DONG - MÓVEL (EM FORMA DE FEIJÃO (ANEURISMA) COM TREMOR AO BATIMENTO (SOPRO)) = CHOQUE EMOCIONAL, DOR EXTREMA, TEPT')
        if tipo_p[25] in pool:
            pool.add('CU - PRECIPITADO (RÁPIDO E INTERROMPIDO EM INTERVALOS REGULARES, TAQUIARRITMIAS) = CALOR EXTREMO OU DEF QI/C')
        if tipo_p[26] in pool:
            pool.add(
                'JIE - NODOSO (LENTO E INTERROMPIDO EM INTERVALOS REGULARES, BRADIARRITMIAS) = FRIO VAZIO EM C')
        if tipo_p[27] in pool:
            pool.add('DAI - INTERMITENTE (NORMOCÁRDICO, INTERROMPIDO EM INTERVALOS REGULARES) = GRAVIDADE DE QI OU XUE EM 2 ÓRGÃOS YIN')
        if tipo_p[28] in pool:
            pool.add(
                'JI - ACELERADO (TAQUICARDIA COM GRANDE FORÇA) = EXCESSO DE YANG COM FOGO EXAURINDO O YIN')
        ...
        try:
            ...
            if ' ' in doc:
                ...
                if len(seta)!=0:
                    ...
                    1 seta = 7 prescrições (14 AGULHAS) (+++)
                    2=14 (28 AGULHAS) (+++)
                    3=21 (42 AGULHAS) (+++)
                    4=28 (56 AGULHAS) (+++)
                    5=22 (44 AGULHAS) (++)
                ...
                if len(pool3) == 0:
                    ...
                    for i, elemento in enumerate(acu2):
                        if elemento in acu2[i+1:]:
                            print(
                                f'PRESCRIÇÃO REPETIDA EM ACUPONTO: {elemento}.')
                    print(
                        '___________________________________________________________\n\n')
