                            ...
                            if os.path.exists("cadastro.csv") == True:
                                if pacientecadastrado == False:
                                    data = [
                                        ...
                                        sexo,
                                        str(expodn),
                                        str(pre_especcodcompa_dn),
                                    ]
                                    with open(
                                        "cadastro.csv", "a", encoding="UTF8", newline=""
                                    ) as f:
                                        writer = csv.writer(f)
            ...
            if "*" in doc:
                if " " in doc:
                    ...
                    for i in range(tamanho):
                        ...
                        if export5 >= int(int(docm[i])**2):
                            export5 -= int(int(docm[i])**2)
                    continue
                elif len(doc) > 1:
                    doc.split()
                    docx = doc[1:]
                    ...
                    if "FIM" in digpre:
                        ...
                        if homm == "A":
                            ...
                            data = [
                                ...
                                str(limpar(pureli)),
                                str(totpul),
                                str(str(limpar(seta))+' / '+str(limpar(dxcid))),
                                str(limpar(pool3)),
                                str(limpar(warn_pun)),
                                str(limpar(questionario)),
                                ver,
                            ]
                            with open(
                                "registro_acupuntura.csv",
                                "a",
                                encoding="UTF8",
                                newline="",
                            ) as f:
                            ...
                            # EXPORT1=LÍNGUA (NUMERADA) ELEVADA AO QUADRADO
                            # ZHANG FU EM SEQUÊNCIA P(SUP/XUE/PROFUNDO)/BP(SUP/XUE/PROFUNDO)/PC/C/F/R(SUP/XUE/PROFUNDO)
                            # EXPORT3=USANDO LETRAS CONVERTIDAS EM NÚMEROS E EXPONENCIADOS AO QUADRADO
                            # EXPORT4=COMPLEIÇÃO 1=C 2=BP 3=P 4=R 5=F 0=SEM COMPLEIÇÃO
                            # EXPORT6(SIM PULOU MESMO!)=EXAME RENYNG 1=C 2=BP 3=P 4=R 5=F 0=NORMAL
                            # EXPORT5=DIAGNÓSTICOS USAR NUMERAÇÃO DE DX PRINCIPAL DO PROGRAMA E NÚMERO EXPONENCIADO AO QUADRADO E SOMADO SE MAIS DE UM (SOMA DAS EXPONENCIAIS) (EVITA SIMULTANEIDADE DE PRODUTOS SOMADOS, E.G. DX1+2+3=6 DX1 E DX5 TAMBÉM DÁ 6 MESMO SENDO OUTROS DX, PORÉM EXPONENCIADOS SEQUENCIALMENTE: 14 E 26)
                            # OBJETIVO: USAR EM SUPPORT MACHINE VECTORS OU REGLOG
                            data = [export1, int(d1a), int(d1b), int(d1c), int(d2a), int(d2b), int(d2c), int(d3a), int(d3b), int(d3c), int(e1a), int(
                                e1b), int(e1c), int(e2a), int(e2b), int(e2c), int(e3a), int(e3b), int(e3c), export3, export4, export6, export5]

                            with open(
                                    "ailog.csv", "a", encoding="UTF8", newline="") as f:
                                writer = csv.writer(f)
                                writer.writerow(data)
                            ...
                            with open(
                                "prontuario.csv", "a", encoding="UTF8", newline=""
                            ) as f:
                                writer = csv.writer(f)
                                writer.writerow(data)
                        if homm == "D":
                            ...
                            data = [
                                ...
                                " ",
                                ver,
                            ]  # anula recomendações e métodos (warn_pun e questionario) ao fim
                            with open(
                                "registro_acupuntura.csv",
                                "a",
                                encoding="UTF8",
        ...
        try:
            ...
            if os.path.exists("registro_acupuntura.csv") == True and os.path.exists("ailog.csv") == True and os.path.exists("prontuario.csv") == True:
                ...
                if os.path.exists("registro_acupuntura.csv") == False:
                    ...
                    with open(
                        "registro_acupuntura.csv", "w", encoding="UTF8", newline=""
                    ) as f:
                        writer = csv.writer(f)
...
atualizações = {
    ...
    'TAO 5.1': 'CORREÇÃO DOS SEGUINTES PROBLEMAS: ERRO SOLICITANDO NOVAMENTE ENDEREÇO AO USAR PINPOINT DEVIDO A USO DE .CSV, ARQUIVO USANDO .TXT POR SER LINHA ÚNICA RESOLVEU PROBLEMA, ERRO DE QUICK==TRUE NÃO PULAR O QUESTIONÁRIO POR CÁLCULOS DE DIFERENÇAS, HDA EM BRANCO SAI TRACEJADA, EXPORT2 GERAVA LISTA COMO STRING PARA BASE DE DADOS (ATRAPALHANDO FUTURAMENTE A BASE DE TREINAMENTO, ADIÇÃO DE INFORMAÇÕES SOBRE AS QUATRO BASES DE DADOS NO INFO, ARMAZENAMENTO DE PINPOINT DE ENDEREÇO RÁPIDO MOVIDO DE RAM PARA ARQUIVO (FICANDO DISPONÍVEL EM SCRIPTS), ERRO EXPORT5 SAI 0 EM VIRTUDE DE SER FUNÇÃO DE COMPREENSÃO DE PYTHON, FINALIZADO TESTES DE BASE DE TREINO DE REGLOG/SVM, A PARTIR DESTA VERSÃO ESTANDO AJUSTADOS PARA COLETA. RESPECTIVAMENTE PARA BASE DE TREINAMENTOS DE ALGORÍTMOS NOS DADOS ABAIXO: EXPORT1=LÍNGUA (NUMERADA) ELEVADA AO QUADRADO, 18 NÚMEROS SEQUENCIAIS CORRESPONDENDO A ZHANG FU EM SEQUÊNCIA P(SUP/XUE/PROFUNDO)/BP(SUP/XUE/PROFUNDO)/PC(SUP/XUE/PROFUNDO)/C(SUP/XUE/PROFUNDO)/F(SUP/XUE/PROFUNDO)/R(SUP/XUE/PROFUNDO), EXPORT3=USANDO LETRAS CONVERTIDAS EM NÚMEROS E EXPONENCIADOS AO QUADRADO, EXPORT4=COMPLEIÇÃO 1=C 2=BP 3=P 4=R 5=F 0=SEM COMPLEIÇÃO, EXPORT6(SIM PULOU MESMO!)=EXAME RENYNG 1=C 2=BP 3=P 4=R 5=F 0=NORMAL, EXPORT5=DIAGNÓSTICOS USAR NUMERAÇÃO DE DX PRINCIPAL DO PROGRAMA E NÚMERO EXPONENCIADO AO QUADRADO E SOMADO SE MAIS DE UM (SOMA DAS EXPONENCIAIS) (EVITA SIMULTANEIDADE DE PRODUTOS SOMADOS, E.G. DX1+2+3=6 DX1 E DX5 TAMBÉM DÁ 6 MESMO SENDO OUTROS DX, PORÉM EXPONENCIADOS SEQUENCIALMENTE: 14 E 26) META DE DADOS: USAR EM SUPPORT MACHINE VECTORS OU REGLOG',
    'TAO 5.1.1': 'CORREÇÃO TEMPORAL DE ADIÇÕES E REDUÇÕES EM DIAS DE 24H CAUSANDO HORAS NEGATIVAS OU ACIMA DE 24H, INSERÇÃO DE SPLITTING DE PULSOS PERMITINDO INSERÇÃO MASSIVA SIMULTÂNEA COM COLETA E ANÁLISE SÍNCRONA EM FUNÇÕES PRÉVIAS'
}
prel = []
