...
def home():
    while True:
        ...
        if homm == 'A' or homm=='D':
            if rede is False:
                cls()
                print('\n\n\n\nATENÇÃO: SEM ACESSO A INTERNET\n\nALGUNS SERVIÇOS SERÃO INDISPONÍVEIS:\nPESQUISA DE CEP\nCONDIÇÕES CLIMÁTICAS')
                time.sleep(5)                
            cls()
            global expli
...
def ben():
    while True:
        ...
        if dorquery == 'S':
            ...
            h5 = input('\n\nDIGITE DOR E LOCAL (AF, AG, DI) OU X SE DISTANTE DE MERIDIANO (ALGORITMO TENDINOMUSCULAR): ').upper()
            if 'X' not in h5 and len(h5)>1:
                ...
                hx = input('\n\nDIGITE TIPO DE DOR E REGIÃO ACOMETIDA (AK, BW, CU): ').upper()
                def to1(x):
                    if x=='AK' or x=='DK':
