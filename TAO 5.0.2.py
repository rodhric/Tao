    ...
    while True:
        try:
            if len(l_f) == 0:
                ...
                per = input(
                    '\n\nDESEJA MANTER OU APAGAR OS DADOS ACIMA? (M/A) ').upper()
                if per == 'M':
                    break
                elif per == 'A':
                    l_f.clear()
                    per2 = input(
                        'APAGADO! DESEJA INSERIR NOVO PONTO FIXO OU RETORNAR AO MENU? (I/R) ').upper()
                    if per2 == 'I':
                        continue
                    elif per2 == 'R':
                        break
                    ...
                    if "FIM" in digpre:
                        ...
                        if homm == "A":
                            ...
                            data = [export1, export2, export3,
                                    export4, export6, export5]
                            with open(
                                "ailog.csv", "a", encoding="UTF8", newline=""
                            ) as f:
                                writer = csv.writer(f)
                                writer.writerow(data)
                            data = [
...
atualizações = {
    ...
    "TAO 5.0.1": "UPDATE COM ERROS, TENTATIVA DE AJUSTES SEM DOWNGRADE DE VERSÃO",
    "TAO 5.0.2": "UPDATE COM ERROS, SUCESSO DE PATCH, EM TESTES, INSERIDO MÓDULO DE ENDEREÇO FIXADO",
}
prel = []
