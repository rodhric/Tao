            ...
            if len(seta) < 1:
                ...
                if digprepre.isnumeric() == True:
                    ...
                    if 'FIM' in digpre:
                        ...
                        if homm=='D':
                            # ARQUIVAMENTO DE DADOS EM DB
                            data=["'"+str(cpf)+"'",str(nome),sexo,str(idd),str(expodn),str(pre_especcodcompa_dn),str(acompanhante.upper()),str(horadia),str(addr),'N/D','N/D','N/D',str(limpar(seta)),str(limpar(pool3)),'N/D','N/D',ver] # anula recomendações e métodos (warn_pun e questionario) ao fim
                            with open('registro_acupuntura.csv', 'a', encoding='UTF8', newline='') as f:
                                writer = csv.writer(f)
                                writer.writerow(data)       
