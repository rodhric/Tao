                ...
                if digprepre.isnumeric() == True:
                    ...
                    if '*' in digpre:
                        ...
                        if ' ' in digpre:
                            ...
                            tam = int(len(mulp))
                            for i in range(tam):
                                oneway.discard(mulp[i])
                                pool3.discard(mulp[i])
                            continue
                        else:
                            mulp = digpre.split()
                            pool3.discard(mulp[1:])
                            oneway.discard(mulp[1:])
                            continue
                    if ' ' in digpre and len(digpre) > 6:
                        cutspe = digpre.split(' ')
                        tamanho = int(len(cutspe))
                        for i in range(tamanho):
                            sli = cutspe[i].split()
                            if sli[1:] not in oneway:
                                pool3.add(cutspe[i])
                                oneway.update(sli[1:])
                        continue
                    if 'FIM' in digpre:
                        ...
                        if homm == 'A':
                            ...
                            print(
                                f'Na sessão foi prescrito os pontos de tratamento: {pool3}, usando como siglas, {met}')
                        ...
                        if len(digpre) < 6:
                            cut = digpre.split()
                            if cut[0] == 'G' or cut[0] == 'H' or cut[0] == 'W' or cut[0] == 'X' or cut[0] == 'Y' or cut[0] == 'Z' or cut[0] == 'K' or cut[0] == 'M' or cut[0] == 'N' or cut[0] == 'A' or cut[0] == 'D' and cut[0:] not in oneway:
                                pool3.add(digpre)
                                oneway.update(cut[1:])
                                print()
                                continue
