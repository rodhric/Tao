    ...
    while True:
        ...
        if dorquery == 'S':
            ...
            if 'B' in h5:
                ...
                if 'BE' in h5:
                    moradia.add('V')
                    h8.add(h6+e)
                    warn_pun.add('B- Localizar foco de dor vazia em '+e)
                if 'BF' in h5:
                    h8.add(h6+f)
                    warn_pun.add('B- Localizar foco de dor vazia em '+f)
                if 'BG' in h5:
                    h8.add(h6+g)
                    warn_pun.add('B- Localizar foco de dor vazia em '+g)
                if 'BH' in h5:
                    h8.add(h6+h)
                    warn_pun.add('B- Localizar foco de dor vazia em '+h)
                if 'BI' in h5:
                    h8.add(h6+i)
                    warn_pun.add('B- Localizar foco de dor vazia em '+i)
                if 'BJ' in h5:
                    h8.add(h6+j)
                    warn_pun.add('B- Localizar foco de dor vazia em '+j)
            if 'C' in h5:
                h6 = 'FRIO EM '
                if 'CE' in h5:
                    h8.add(h6+e)
        ...
        while True:
            try:
                ...
                if comp_so == 4:
                    ...
                    a = True
                if comp_so == 5:
                    warn.add('C- DETECTADO ALTERAÇÃO LOCALIZADA EM: MÚSCULO')
                    moradia.add('M')
                    a = True
                if comp_so == 6:
        ...
        while True:
            try:
                ...
                if emo == 1:
                    warn.add(
                        'A- ATENÇÃO! DETECTADO ALTERAÇÃO LOCALIZADA EM: CANAL')
                    moradia.add('C')
                    break
                if emo == 2:
                    warn.add('I- DETECTADO ALTERAÇÃO LOCALIZADA EM: TENDÃO')
                ...
                if emo == 3:
                    warn.add(
                        'C- ATENÇÃO! DETECTADO ALTERAÇÃO LOCALIZADA EM: MÚSCULO')
                    moradia.add('M')
                    break
                if emo == 4:
                    warn.add(
        ...
        if 'C' in moradia:
            warn_pun.add(
                'A- Aplicação contra-lateral a dor (esta deverá existir em meridiano afetado)')
            warn_pun.add(
                'A- Sangramento de colateral é indicado caso canal afetado com colateral congesto')
            warn_pun.add(
                'A- Liberado método de lunação para tratamento de síndrome bi de canal afetado')
            warn_pun.add(
                'A- Sintomas de vísceras deverão ser tratados via pontos yuan')
            warn_pun.add('A- Pode ser usado agulha de fogo')
        if 'V' in moradia:
            warn_pun.add('B- Aplicação rápida de agulha')
            warn_pun.add(
                'B- Segurar com mâo esquerda apertando após picar, e, então retirar a agulha com a mão empurrando')
            warn_pun.add(
                'B- Repetir em locais de dor (ou em mesmo local) até melhorar')
        if 'M' in moradia:
            warn_pun.add('C- Procurar meridiano com queixa de dor')
            warn_pun.add(
                'C- 3 picadas em mesmo local em forma de pé-de-galinha, central e duas divergentes em mesmo acuponto')
        if 'O' in moradia:
            warn_pun.add(
                'D- Insersão por soerguimento delicada até osso, em linha reta')
            warn_pun.add('D- Insersão em tecido conectivo do osso de junta')
        if 'J' in moradia:
            warn_pun.add(
                'F- Aplicar puntura em lados internos de coxa bilateralmente, em associação com R3')
        if 'A' in moradia:
            warn_pun.add(
                'E- Procurar nódulo/tumor/edema em meridiano com umidade')
            warn_pun.add(
                'E- Agulhar local e outra face do local (e.g. dorso de mão e palma de mão ou peito e costas)')
        if 'P' in moradia:
            warn_pun.add('G- Puntura superficial, rasa e rápida')
        if 'L' in moradia:
            warn_pun.add(
                'H- Delimitar área de colateral estagnado, punção sob delimitação desenhada')
        if 'T' in moradia:
            warn_pun.add('I- Proximidade em junta com dor ou em IFD/IFP/MCF')
            warn_pun.add(
                'I- Sem cruzar lados, se dor em local de dor (fora de canal), devendo sangrar')
...
# -------------------------------------- VERIFICAÇÃO DE DIAGNÓSTICO - PARTE 4
        ...
        if len(pool) >= 1:
            ...
            print(pool)
        print('PULSOLOGIA DA ANÁLISE:\n\n')
        print('D1A:'+d1a+' D1B:'+d1b+' D1C:'+d1c+'\n D2A:'+d2a+' D2B:'+d2b+' D2C:'+d2c+'\n D3A:'+d3a+' D3B:'+d3b+' D3C:'+d3c +
              '\n E1A:'+e1a+' E1B:'+e1b+' E1C:'+e1c+'\n E2A:'+e2a+' E2B:'+e2b+' E2C:'+e2c+'\n E3A:'+e3a+' E3B:'+e3b+' E3C:'+d3c+'\n')
    ...
    while True:
        try:
            ...
            if ' ' in doc:
                ...
                def sprap(x, y=' '):
                    if dx[0] in x:
                        a1 = 'SC7 SVC14 SVC15 SVC4 WQB17 SB15 SB20'+y
                    else:
                        a1 = ''
                    if dx[6] in x or dx[174] in x:
                        a2 = 'SID2 SID5 SC5 SC8 SE39'+y
                    else:
                        a2 = ''
                    if dx[12] in x or dx[186] in x or dx[18] in x:
                        a3 = 'SVC6 SVB34 SF13 SE29 SE27 SBP6 SF3'+y
                    else:
                        a3 = ''
                    if dx[168] in x:
                        a4 = 'SID2 SID5 SC5 SC8 SE39'+y
                    else:
                        a4 = ''
                    if dx[180] in x:
                        a5 = 'WQC5 WQPC6 WQB15 WQVC17 WQVC6 WQVG14'+y
                    else:
                        a5 = ''
                    ...
                    if int(est) > 1:
                        a7 = 'SR14 SF3 SB16 SBP10 SB17 SVC3 SE28 SB39 SB22 SBP6'+y
                    else:
                        a7 = ''
                    if dx[107] in x or dx[143] in x or dx[173] in x and dx[59] in dxconff:
                        a8 = 'SF2 SBP10 SF3 SR2 SBP11'+y
                    else:
                        a8 = ''
                    if dx[253] in x in dxconff:
                        ...
                        a8 = ''
                    if dx[61] in x:
                        b1 = 'WE36 WB6 WVC12 SVC10 SE34 SE21 SE19'+y
                    else:
                        b1 = ''
                    if dx[55] in x:
                        b2 = 'WVC12 WE36 WBP10 WBP3 WBP6 WB20 WQB17 SE34 SE21'+y
                    else:
                        b2 = ''
                    if dx[1] in x:
                        b3 = 'WQVC12 WQE36 WBP3 SBP1 SBP6 SBP10'+y
                    else:
                        b3 = ''
                    if dx[7] in x or dx[175] in x:
                        b4 = 'WVC12 WQE36 WBP6 WBP3 SBP10'+y
                    else:
                        b4 = ''
                    if dx[13] in x or dx[187] in x or dx[19] in x:
                        b5 = 'WQVC12 WQE36 WQBP3 WQBP6 WQB20 WQB21 WQBP9 WQE28 WQVC9 WP9'+y
                    else:
                        b5 = ''
                    if dx[169] in x:
                        b6 = 'SBP2 SBP9 SBP6 SVG9 SIG11 WB20 SVB34 SVC9 SVC11 SE22 SE28'+y
                    else:
                        b6 = ''
                    if dx[181] in x:
                        b7 = 'WVC12 WE36 WQBP6'+y
                    else:
                        b7 = ''
                    if dx[229] in x:
                        b8 = 'SIG4 SIG11 STA5 SVG14 SB12 YP11'+y
                    else:
                        b8 = ''
                    if dx[230] in x:
                        b9 = 'SIG4 SIG11 STA5 SVG14 SVG26 SB40 SPC9'+y
                    else:
                        b9 = ''
                    if dx[231] in x:
                        b10 = 'SIG4 SIG11 SBP8 SBP6 SVC12 SVC9'+y
                    else:
                        b10 = ''
                    if dx[232] in x:
                        b11 = 'SIG4 SIG11 STA5 SBP6 SP9 SVC12 SE36'+y
                    else:
                        b11 = ''
                    if dx[233] in x:
                        b12 = 'SP5 SP10 SVG14 SIG11 SP1 SB13'+y
                    else:
                        b12 = ''
                    if dx[234] in x:
                        b13 = 'SE44 SE34 SE21 SE43 SIG11 SE25'+y
                    else:
                        b13 = ''
                    if dx[235] in x:
                        b14 = 'SIG11 SE25 SBP15 SE37 SE39'+y
                    else:
                        b14 = ''
                    if dx[236] in x:
                        b15 = 'SVB34 SVB43 STA6 STA5'+y
                    else:
                        b15 = ''
                    if dx[237] in x:
                        b16 = 'SVC12 SBP9 SBP6 SVC9 SE36 SIG11 SB20 SB22'+y
                    else:
                        b16 = ''
                    if dx[238] in x:
                        b17 = 'SPC9 SPC8 SC9 SR6 SEX17'+y
                    else:
                        b17 = ''
                    if dx[239] in x:
                        b18 = 'SPC9 SPC3 SPC8 SC9 SR6 SIG11'+y
                    else:
                        b18 = ''
                    if dx[240] in x:
                        b19 = 'SB17 SBP10 SF5 SBP4 SIG11 SF2 SR6 SC9'+y
                    else:
                        b19 = ''
                    if dx[241] in x:
                        b20 = 'SBP10 SIG11 SF2 SR6 SC9 SF3 SVG16 SVG20 SID3 SB62'+y
                    else:
                        b20 = ''
                    if dx[242] in x:
                        b21 = 'SF3 SVG16 SVB20 SID3 SB62 SF8 SR6 SR3 SBP6'+y
                    else:
                        b21 = ''
                    if dx[243] in x:
                        b22 = 'SE36 SR3 SBP6 SR6 SVC4'+y
                    else:
                        b22 = ''
                    if dx[244] in x:
                        b23 = 'SE36 SVC6 SVC4 SVC8'+y
                    else:
                        b23 = ''
                    if dx[245] in x:
                        b24 = 'SIG4 SIG11 STA5 SVG14 YB12 SP11'+y
                    else:
                        b24 = ''
                    if dx[246] in x:
                        b25 = 'SP5 SP10 SP1 SIG11 SB13'+y
                    else:
                        b25 = ''
                    if dx[247] in x:
                        b26 = 'SPC9 SPC3 SIG11 SPC8 SC9 SR6 SEX17'+y
                    else:
                        b26 = ''
                    if dx[248] in x:
                        b27 = 'SE44 SE34 SE21 SE43 SIG11 SE25'+y
                    else:
                        b27 = ''
                    if dx[249] in x:
                        b28 = 'SVC12 SBP9 SBP6 SVC9 SE36 SIG11 SB20 SB22'+y
                    else:
                        b28 = ''
                    if dx[250] in x:
                        b29 = 'SR2 SR3 SR6 SBP6 SIG11'+y
                    else:
                        b29 = ''
                    if dx[251] in x:
                        b30 = 'SF2 SF3 SVB20 SVG16 SID3 SB62 SBP10 SIG11 SR6 SC9 SEX17 SB62'+y
                    else:
                        b30 = ''
                    if dx[252] in x:
                        b31 = 'SF2 SF3 SVB20 SVG16 SID3 SB62 SR3 SR6 SBP6 SF8'+y
                    else:
                        b31 = ''
                    if dx[254] in x:
                    ...
                    est = umidade(x)
                    if int(est) > 1:
                        b33 = 'WVC12 WE36 WB20 WBP9 WBP6 SPC5 SE40 SBP3 WE40'+y
                    else:
                        b33 = ''
                    if dx[202] in x:
                        c1 = 'SVC3 SR14 SE28 SB39 SB22 SBP10 SF3 SBP6'+y
                    else:
                        c1 = ''
                    if dx[203] in x:
                        c2 = 'SIG11 SVG14 SPC3 SE44 SE25 SBP15 SE37 SBP6'+y
                    else:
                        c2 = ''
                    if dx[204] in x:
                        c3 = 'STA5 STA6 SVB41 SVG13'+y
                    else:
                        c3 = ''
                    if dx[205] in x:
                        c4 = 'TQVC12 TQB20 TQE36 TQE25 TQBP6'+y
                    else:
                        c4 = ''
                    if dx[206] in x:
                        c5 = 'SVC4 SVC6 SR3 SR6 SBP6'+y
                    else:
                        c5 = ''
                    if dx[207] in x:
                        c6 = 'SF3 SIG4 SBP4 SPC6'+y
                    else:
                        c6 = ''
                    if dx[63] in x:
                        c7 = 'SP7 WVC7 SB12 SB13'+y
                    else:
                        c7 = ''
                    if dx[57] in x:
                        c8 = 'SP7 SIG4 SIG23 WVC6 WE25 WE36 WBP3'+y
                    else:
                        c8 = ''
                    if dx[3] in x:
                    ...
                    if dx[9] in x or dx[177] in x:
                        c10 = 'SP10 SIG11 WP9 WVC17 WB43 WB13 WVG12 WR6'+y
                    else:
                        c10 = ''
                    if dx[15] in x or dx[189] in x or dx[21] in x:
                        c11 = 'WP7 SIG4 WQVC6 WQE25 WP9 WQE36 WQE37 WQB20'+y
                    else:
                        c11 = ''
                    if dx[171] in x:
                        c12 = 'SP10 SP5 SIG11 WVC17 WP9 WE36 WP7 WQVC6 WQB13 WQVG12'+y
                    else:
                        c12 = ''
                    if dx[183] in x:
                        c13 = 'SE37 SE25 SE36 SBP6 SF3 SE27'+y
                    else:
                        c13 = ''
                    if dx[255] in x:
                        c14 = 'WB42 WP7'+y
                    ...
                    est = secura(x)
                    if int(est) > 1:
                        c15 = 'WVC4 WP9 WP7 WR6 SVC12 SE36 SBP6 SBP6 SE40 SB13'+y
                    else:
                        c15 = ''
                    ...
                    est = yuan(x)
                    if int(est) > 1:
                        d1 = 'WR3 WF3 WC7 WBP3 WP9 WVG4 WVC4 WQVC7 WQVC6 WVC5 WQVC4'+y
                    else:
                        d1 = ''
                    if dx[64] in x:
                    ...
                    if dx[16] in x or dx[190] in x or dx[22] in x:
                        d6 = 'WQR3 WQB23 WQVG4 WQVC4 WQVC6 WQR7 WQB52'+y
                    else:
                        d6 = ''
                    if dx[172] in x:
                        d7 = 'SBP9 SBP6 SB22 SB28 SVC3 SB63 SB60 SE28'+y
                    else:
                        d7 = ''
                    if dx[184] in x:
                        d8 = 'WB23 WVG4 SQBP9 SQBP6 SQB22 SQE28 SQVC3 SQVC9'+y
                    else:
                        d8 = ''
                    if dx[256] in x:
                        d9 = 'WB52 WE37'+y
                        ...
                        d9 = ''
                    if dx[65] in x:
                        e1 = 'SP7 SF3 SVC17 SF2 SP5 SVB34 '+y
                    else:
                        e1 = ''
                    if dx[59] in x:
                        e2 = 'SF14 SVB34 SF3 SB18 SB17 SBP10 SBP6 SBP4 SPC6 SE29'+y
                    else:
                        e2 = ''
                    if dx[5] in x:
                        e3 = 'WE36 SF8 SBP6 WE36 WVC4 WB18'+y
                    else:
                        e3 = ''
                    if dx[11] in x or dx[179] in x:
                        e4 = 'SF2 SF8 SBP6 SE36 SVC4 SR6 SR3'+y
                    else:
                        e4 = ''
                    if dx[17] in x or dx[191] in x or dx[23] in x:
                        ...
                        e5 = ''
                    if dx[173] in x:
                        e6 = 'SF2 SF3 SVB20 SVB13 SIG11 SVB1 SVB9 SVB8 SVB6 SBP6 SF1'+y
                    else:
                        e6 = ''
                    if dx[185] in x:
                        e7 = 'SVC3 SF5 SF1 SF3'+y
                    else:
                        e7 = ''
                    if dx[257] in x:
                        e8 = 'WB47 SF3'+y
        ...
        try:
            if len(seta) < 1:
                ...
                if len(pool3) == 0:
                    ...
                    print(f'\n\nAO MOMENTO ADICIONADOS: {pool3}.\n\n')
                    sed = [item[1:] for item in pool3 if 'S' in item]
                    if len(sed) != 0:
                        print('\nPONTOS SEDADOS:')
                        print(sorted(sed))
                    ton = [item[1:] for item in pool3 if 'W' in item]
                    if len(ton) != 0:
                        print('\nPONTOS TONIFICADOS:')
                        print(sorted(ton))
                    mox = [item[1:] for item in pool3 if 'Q' in item]
                    if len(mox) != 0:
                        print('\nPONTOS COM MOXA:')
                        print(sorted(mox))
                    ...
                    if len(san) != 0:
                        print('\nPONTOS COM SANGRIA:')
                        print(sorted(san))
                    dir = [item[1:] for item in pool3 if 'M' in item]
                    if len(dir) != 0:
                        print('\nPONTOS UNILATERAIS DIREITA:')
                        print(sorted(dir))
                    esq = [item[1:] for item in pool3 if 'N' in item]
                    if len(esq) != 0:
                    ...
                    if len(esq) != 0:
                        agu -= len(esq)
                    u = [
                        item[1:] for item in pool3 if 'SEX' or 'WEX' or 'QEX' or 'ZEX' or 'KEX' or 'YEX' or 'MEX' or 'NEX' in item]
                    if len(u) != 0:
                        if 'EX1' in u:
                            agu += 2
                        if 'EX12' in u:
                        ...
                        if len(warn_pun) != 0:
                            ...
                            for i, j in enumerate(list(warn_pun), start=0):
                                print(i, j)
                ...
                if digprepre.isnumeric() == True:
                    ...
                    if 'FIM' in digpre:
                        ...
                        if len(digpre) >= 3 and digpre != 'FIM' or len(digpre) < 7:
                            cut = list(digpre)
                            if cut[0] == 'S' or cut[0] == 'W' or cut[0] == 'Q' or cut[0] == 'Z' or cut[0] == 'Y' or cut[0] == 'K' or cut[0] == 'M' or cut[0] == 'N':
                                pool3.add(digpre)
                                print()
                                continue
                            else:
    ...
    while True:
        try:
            ...
            if pesq == 'FIM':
                ...
                filt = [i for i in cid11 if i.count(pesq) > 0]
                print(filt)
                print()
                print()
                continue
...
if estação == 'PRIMAVERA':
    warn_pun.add('- Picar colaterais de canal e atingir porfundamente entre músculos, se gravidade picar profundamente - estação de primavera, fonte: si shi qi de ling shu')
    warn_pun.add('H- Tratar colateral engurgitado')
    moradia.add('L')
if estação == 'VERÃO':
    warn_pun.add('- Picar canais yang (dan, dachang, wei, xiaoxang, panguan, sanjiao) e colaterais do ponto shu, sendo picado somente superficialmente - estação de verão, fonte: si shi qi de ling shu')
if estação == 'OUTONO':
    warn_pun.add('- Picar pontos shus de doenças yin ou pontos he de doenças yang, esfregar pele antes de aplicar e aplicar superficialmente - estação de outono, fonte: si shi qi de ling shu')
if estação == 'INVERNO':
    warn_pun.add('- Picar canas afetados usar pontos poço e xing, picar profundamente em todas punturas e permanecer tempo longo de agulha - estação de inverno, fonte: si shi qi de ling shu')
global horashu
...
global met
met = {'S': 'SEDAÇÃO', 'W': 'TONIFICAÇÃO', 'Q': 'MOXA', 'Z': 'NEUTRO', 'Y': 'VENTOSA',
       'K': 'SANGRIA', 'M': 'SOMENTE À DIREITA DO CORPO', 'N': 'SOMENTE À ESQUERDA DO CORPO'}
global cls
