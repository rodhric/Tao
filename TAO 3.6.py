        ...
        if rede is True:
            ...
            if rede is True:
                try:
                    ...
                    ss_hr=int(tt6)-3
                    c6=f'; PÔR DE SOL: {ss_hr}H{ss_min}; '
                    ...
                    try:
                        ...
                        if pacienteantigo == False:
                            ...
                            if df.loc[df['CPF'].isin(["'"+str(cpf)+"'"])].empty:
                                ...
                                print(dbend)    
                                pulo=input('\nO endereço acima é o local de atendimento? (S/N) ').upper()    
                                if pulo!='S':                        
                                    cep = input('\nDigite o CEP: ')
                                    if len(cep) == 8 and cep.isnumeric() == True:
                                        try:
...
# -------------------------------------- VERIFICAÇÃO DE DIAGNÓSTICO - PARTE 4
        ...
        if tipo_p[1] in pool:
            if tipo_p[16] in pool:
                dxconff.add(str(dx[266]+' '+dx[202]).upper())
            if tipo_p[13] in pool or tipo_p[14] in pool:
                dxconff.add(str(dx[265]+' '+dx[202]).upper())
            else:
                ...
                if ne1=='S':
                    ...
                    if ne2=='B':
                        dxconff.add(str(dx[265]+' '+dx[202]).upper())
                    if ne2=='C':
                        ne3=input('\nSoa muito(A) ou pouco(B)?\n\n▶ ').upper()                        
                        if ne3=='A':
                            dxconff.add(str(dx[267]+' '+dx[202]).upper())
                        if ne3=='B':
                            dxconff.add(str(dx[268]+' '+dx[202]).upper())                        
        ...
        if tipo_p[28] in pool or tipo_p[1] in pool and tipo_p[14] in pool:
            ...
            if ne4=='S':
                ...
                if ne5=='S':
                    dxconff.add(str(dx[266]+' '+dx[203]).upper())
                elif ne5=='N':
                    dxconff.add(str(dx[265]+' '+dx[203]).upper())
        ...
        if tipo_p[2] in pool or tipo_p[14] in pool:
            ...
            if ne6=='S' or ne7=='S':
                if ne7=='S':
                    ...
                    if ne8=='S':
                        dxconff.add(str(dx[266]+' '+dx[204]).upper())
                    elif tipo_p[15] in pool or tipo_p[12] in pool:
                        dxconff.add(str(dx[264]+' '+dx[204]).upper())
                    else:
        ...
        if tipo_p[9] in pool or tipo_p[21] in pool or ne10=='S':
            ne11=input('\nIrritação diária com diarréias que dura menos de 1 semana e passa sozinha porém recorrente? (S/N)\n\n▶ ').upper()
            if ne11=='S':
                dxconff.add(str(dx[265]+' '+dx[205]).upper())
            ne12=input('\nDor em braços ou pernas, como se estivessem cansadas (sendo muito frequente o sintoma)? (S/N)\n\n▶ ').upper()
            if ne12=='S':
                dxconff.add(str(dx[266]+' '+dx[205]).upper())
        ...
        if tipo_p[13] in pool or tipo_p[12] in pool or ne10=='S':
            ...
            ne14=input('\nTem tendência de irritar sem controle no momento e geralmente, em momentos de calma diários, existe uma vontade de vomitar que não é bem náusea? (S/N)\n\n▶ ').upper()
            if ne13=='S' or ne14=='S':
                dxconff.add(str('hang Han Lun/ '+dx[206]).upper())                
        ...
        if 'DEFICIÊNCIA YANG DE TERRA' in h3 or 'DEFICIÊNCIA DE XUE' in h3 or ne15=='S' or ne16=='S':
            if tipo_p[13] in pool:
                dxconff.add(str(dx[266]+' '+dx[207]).upper())
            else:
                ...
                if ne17=='S':
                    dxconff.add(str(dx[265]+' '+dx[207]).upper())
        # WEN BING XUE - YE TIAN SHI
        if tipo_p[1] in pool or tipo_p[2] in pool or 'saburra branca'.upper() in puereli:
            ne19=input('\nCorpo sempre quente com incômodo em locais frios? (S/N)\n\n▶ ').upper()
            if ne19=='S':            
                dxconff.add(str(dx[198]).upper())
        if tipo_p[9] in pool or tipo_p[11] in pool or tipo_p[15] in pool or 'saburra branca'.upper() in puereli or 'língua roxa-escura'.upper() in puereli or 'saburra amarelo-laranja'.upper() in puereli:
            if 'xerostomia'.upper() not in puereli:
                dxconff.add(str(dx[199]).upper())
            else:
                ne20=input('\nCorpo sempre quente com incômodo em locais muito quentes? (S/N)\n\n▶ ').upper()                
                if ne20=='S':            
                    dxconff.add(str(dx[199]).upper())                
        if tipo_p[14] in pool or tipo_p[4] in pool or tipo_p[12] in pool or 'língua roxa-escura'.upper() in puereli:
            if 'sialorréia'.upper() not in puereli:
                dxconff.add(str(dx[200]).upper()) 
            else:
                ne21=input('\nCorpo quente de noite (e somente de noite)? (S/N)\n\n▶ ').upper()                
                if ne21=='S':            
                    dxconff.add(str(dx[200]).upper())                                
        if 'língua branca'.upper() in puereli or 'dura'.upper() in puereli or 'saliva pegajosa'.upper() in puereli or 'língua azul-branca'.upper() in puereli:
            ne18=input('\nCalor em palmas de mãos com grande incômodo? (S/N)\n\n▶ ').upper()
            if ne18=='S':
                dxconff.add(str(dx[201]).upper())
        ...
        if str(dx[201]).upper() in dxconff or str(dx[200]).upper() in dxconff or str(dx[199]).upper() in dxconff or str(dx[198]).upper() in dxconff:
            if tipo_p[4] in pool:
                if tipo_p[1] in pool or 'saburra branca'.upper() in puereli:
                    dxconff.add(str(dx[258]+' COM '+dx[195]).upper())
                if 'saburra amarelo-laranja'.upper() in puereli:
                    dxconff.add(str(dx[258]+' COM '+dx[195]).upper())
            if 'dura'.upper() in puereli:
                dxconff.add(str(dx[259]+' COM '+dx[195]).upper())
            if 'xerostomia'.upper() in puereli or 'saburra amarelo-laranja'.upper() in puereli:
                if tipo_p[1] in pool or tipo_p[9] in pool:
                    dxconff.add(str(dx[260]+' COM '+dx[196]).upper())
                if tipo_p[2] in pool or tipo_p[19] in pool:
                    dxconff.add(str(dx[261]+' COM '+dx[196]).upper())
            if tipo_p[11] in pool or tipo_p[16] in pool or 'saliva pegajosa'.upper() in puereli:
                dxconff.add(str(dx[262]+' COM '+dx[196]).upper())
            if tipo_p[17] in pool:
                if tipo_p[13] in pool or 'língua roxa-escura'.upper() in puereli or 'falha de preenchimento total de saburra'.upper() in puereli:
                    dxconff.add(str(dx[264]+' COM '+dx[197]).upper())
                elif 'sialorréia'.upper() not in puereli:
                    dxconff.add(str(dx[263]+' COM '+dx[197]).upper())
        # FIM DE ANÁLISE DE WEN BING E SHANG HAN LUN, APAGANDO PARÂMETROS DESTE PONTO EM DIANTE                
